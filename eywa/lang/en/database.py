...
But we only require a few key type and value type combinations,
so no need of all the pickling.
We have multiple classes for various edge cases
this is to avoid checks in get/set.
'''
...
class _Database(object):
    ...
    def __init__(self, file, key_type=str, value_type=str, new=False):
        ...
        if sql_value_type is None:
            raise Exception(
                'Unsopported value type {}. \
                Supported value types are {}.'.format(
                    str(value_type), str(
                        type_map.keys())))
        if new:
    ...
    def __contains__(self, key):
        value = self.cursor.execute(
            "SELECT v FROM dict WHERE k = ?", (key,)).fetchone()
    ...
    def __setitem__(self, key, value):
        self.cursor.execute(
            "REPLACE INTO dict (k, v) VALUES (?, ?)", (key, value))
