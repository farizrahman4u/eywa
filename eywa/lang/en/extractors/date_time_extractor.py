...
month_names = month_names_long + '|jan|feb|mar|apr|may|jun|jul|\
                    aug|sep|oct|nov|dec'
...
numbers = r"(^a(?=\s)|one|two|three|four|five|six|seven|eight|nine|ten| \
                    ...
                    |first|second|third|forth|fourth|fifth)"
re_dmy = '(' + "|".join(day_variations + minute_variations +
                        year_variations + week_variations +
                        month_variations) + ')'
re_duration = r'(before|after|earlier|later|ago|from\snow)'
re_year = r"(17|18|19|20)\d{2}|^(17|18|19|20)\d{2}"
re_timeframe = r'current|this|coming|next|following|previous\
                |last|end\sof\sthe|since'
re_ordinal = 'st|nd|rd|th|first|second|third|fourth|fourth|' + re_timeframe
re_number_end = ['st', 'nd', 'rd', 'th']
re_time = r'(?P<hour>\d{1,2})(\s?)(\:(?P<minute>\d{1,2})(?P<convention>am|pm))'
re_separator = 'of|at|on'
all_numbers = numbers + \
    '|first|second|third|forth|fourth|fifth|sixth|seventh|eighth|ninth|tenth'

# A list tuple of regular expressions / parser fn to match
# The order of the match in this list matters, So always start with the
# widest match and narrow it down
regex = [
    # Houndify bug fix
    (re.compile(
        r'''
        (\b)
        (
            ((?P<number_1>\d+|(%s[-\s]?)+))? # Matches any number or string
                                             # 25 or twenty five
            (\s)?
            (%s\s)?
            (\s)?
        ...
        (\b)
        ''' % (numbers, re_ordinal, re_separator, month_names,
               all_numbers, re_ordinal, all_numbers, re_ordinal),
    ...
    (re.compile(
        ...
        (\b)
        (
            ((?P<dow>%s)[,\s]\s*)? #Matches Monday, 12 Jan 2012,
                                   # 12 Jan 2012 etc
        ...
        (\b)
        ''' % (day_names, re_ordinal, re_separator, month_names,
               re_year, re_separator, re_time),
        (re.VERBOSE | re.IGNORECASE)
    ),
    ...
    (re.compile(
        r'''
        (
            ((?P<dow>%s)[,\s][-\s]*)? #Matches Monday, Jan 12 2012,
                                      # Jan 12 2012 etc
            (\b)
            (?P<month>%s) # Matches any month name
            [-\s] # Space
        ...
        ''' % (day_names, month_names, re_ordinal,
               re_year, re_separator, re_time),
        (re.VERBOSE | re.IGNORECASE)
    ),
        lambda m, base_date: (datetime(
    ...
    (re.compile(
        ...
        (\b)
        (
            ((?P<number>\d+|(%s[-\s]?)+)\s)? # Matches any number or string
                                             # 25 or twenty five
    ...
    (re.compile(
        ...
        (
            ...
            ((?P<number>\d+|(%s[-\s]?)+)\s)? # Matches any number or
                                             # string 25 or twenty five
            (?P<unit>%s)s
        )
        (\b)
    ...
    (re.compile(
        ...
        lambda m, base_date: (datetime(
            int(m.group('year') if m.group('year') else base_date.year),
            int(hashmonths[m.group('month').lower()]
                if m.group('month') else 1),
    ...
    (re.compile(
        ...
        base_date, m.group('number'), m.group('month'), m.group('year'))
    ),
    (re.compile(
        r'''
        (\b)
    ...
    (re.compile(
        ...
        lambda m, base_date: dateFromWordsMonthYear(
        base_date, m.group('number'), m.group('month'), base_date.year)
    ),
    (re.compile(
        r'''
        (\b)
    ...
    (re.compile(
        ...
        lambda m, base_date: dateYearRange(
        base_date, m.group('year_start'), m.group('year_end'))
    ),
    (re.compile(
        r'''
        (\b)
...
def convert_string_to_number(value):
    ...
    if value.isdigit():
        return int(value)
    num_list = map(
        lambda s: convert_word_to_number(s),
        re.findall(
            numbers + '\\b',
            value,
            re.IGNORECASE))
...
def dateFromRelativeWeekYear(base_date, time, dow, ordinal=None):
    ...
    # Reset date to start of the day
    # ordinal = int(ordinal) if ordinal is not None
    #           and ordinal.isdigit() else 1
    # Convert to lower
