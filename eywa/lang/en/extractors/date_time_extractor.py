...
month_names_long = 'january|february|march|april|may|june|july|august|september| \
...
re_dmy = '(' + "|".join(day_variations + minute_variations +
                        year_variations + week_variations + month_variations) + ')'
re_duration = r'(before|after|earlier|later|ago|from\snow)'
re_year = r"(17|18|19|20)\d{2}|^(17|18|19|20)\d{2}"
re_timeframe = r'current|this|coming|next|following|previous|last|end\sof\sthe|since'
...
regex = [
    ...
    (re.compile(
        ...
        ''' % (numbers, re_ordinal, re_separator, month_names, all_numbers,
               re_ordinal, all_numbers, re_ordinal), (re.VERBOSE | re.IGNORECASE)
    ),
    ...
    (re.compile(
        ...
        (\b)
        ''' % (day_names, re_ordinal, re_separator, month_names, re_year,
               re_separator, re_time), (re.VERBOSE | re.IGNORECASE)
    ),
