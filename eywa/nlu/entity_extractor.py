...
class EntityExtractor(object):
    ...
    def fit(self, X, Y):
        ...
        Train an `EntityExtractor` to extract entities
        "intent" and "place" from utterances
        labels "intent" and "place":
        ```python
        x = ['who was the first president of USA',
            'which party got elected last time']
    ...
    def predict(self, x, keys=None, return_scores=False):
        ...
        If `True`, Returns a dict (or list thereof if x is a list)
        mapping entity names to a list of tuples, where each tuple
        consists of a possible entity value (`str`) and a confidence score (`float`).
        Else, returns entity name for entity value with highest confidence
        per utterance.
