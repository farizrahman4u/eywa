...
def test_classifier_server_get():
    ...
    r = server.requests.get(
        "/models/classifier/predict?input=will it rain today")
...
def test_entity_extractor_server_get():
    ...
    input_ = "what is the weather in london like"
    r = server.requests.get(
        "/models/entity_extractor/predict?input=%s" %
        (input_))
...
def test_classifier_server_train_get():
    ...
    for y, v in data.items():
        for x in v:
            server.requests.get(
                "/models/classifier/train?input={}&target={}".format(x, y))
            clf.fit(x, y)
    r = server.requests.get(
        "/models/classifier/predict?input=will it rain today")
    assert r.text == 'weather'
...
def test_entity_extractor_server_train_get():
    ...
    for x, y in zip(X, Y):
        r = server.requests.get(
            "/models/entity_extractor/train?input={}&target={}".format(
    ...
    time.sleep(2)
    r = server.requests.get(
        "/models/entity_extractor/predict?input={}".format(test_inp))
